...
def parse_args():
    ...
    parser.add_argument("--model_path", type=str, default="./outputs", help="LoRA模型路径")
    parser.add_argument("--base_model", type=str, default="Qwen/Qwen2-VL-2B-Instruct", help="基础模型路径")
    parser.add_argument("--output_file", type=str, default="./results/lora_evaluation_results.json", help="结果输出文件")
    parser.add_argument("--batch_size", type=int, default=4, help="批处理大小")
    parser.add_argument("--max_samples", type=int, default=500, help="最大样本数，设为None评估全部样本")
...
class SimpleRewardModel(torch.nn.Module):
    ...
    def __init__(self, base_model_path, lora_model_path):
        ...
        # 如果存在score_head.pt文件，加载它
        score_head_path = os.path.join(lora_model_path, "score_head.pt")
        if os.path.exists(score_head_path):
            ...
            logger.warning(f"未找到score_head权重文件: {score_head_path}")
...
def evaluate_model(args):
    ...
    if args.wandb_name:
        wandb.init(
            ...
            tags=["reward-model", "evaluation", "lora"],
            notes=f"评估LoRA模型: {args.model_path}"
        )
    
    # 加载模型
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = SimpleRewardModel(args.base_model, args.model_path)
    model.eval()
    ...
    processor = Qwen2VLProcessor.from_pretrained(args.base_model, trust_remote_code=True)
